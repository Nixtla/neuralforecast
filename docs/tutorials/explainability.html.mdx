...
Notice how the shape of the attribution tensors is different from the
univariate case. Take the insample attribution for example. In the
univariate case, the shape `[2, 12, 1, 1, 24, 2]` shows that we have two
batches with 1 series in each, because we are forecasting each series
independently. However, for the multivariate case, the shape is
`[1, 12, 2, 1, 24, 2, 2]`. Now, we have a single batch, but 2 series,
since we jointly predict both series.

As such, the logic to visualize attribution scores changes slightly for
multivariate models.
