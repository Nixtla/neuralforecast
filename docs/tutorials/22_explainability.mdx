...
2025-09-16 16:17:34,356 INFO util.py:154 -- Missing packages: ['ipywidgets']. Run `pip install -U ipywidgets`, then restart the notebook server for rich notebook output.
2025-09-16 16:17:34,429 INFO util.py:154 -- Missing packages: ['ipywidgets']. Run `pip install -U ipywidgets`, then restart the notebook server for rich notebook output.
```
...
features, read [this tutorial](../capabilities/03_exogenous_variables).
...
2.5 M     Total params
10.003    Total estimated model params size (MB)
...
None. - `series`: List of series to explain for multivariate models.
Defaults to \[0\] (first series). Multivariate models are not supported
yet. - `outputs`: List of outputs to explain for models with multiple
...
![](22_explainability_files/figure-markdown_strict/cell-13-output-1.png)
...
| Kernel SHAP            | Local        | Yes                 | Slow      |
| Shapley Value Sampling | Local        | Yes                 | Very slow |
| LIME                   | Local        | No                  | Medium    |
...
**Additivity Property**: Whether the sum feature attributions and
...
(Integrated Gradients) - Medium: Multiple model evaluations (model
fitting for LIME) - Slow: Many model evaluations for sampling-based
methods - Very Slow: Exponential complexity in worst case (exact Shapley
values)
...
Kernel SHAP computes SHAP (SHapley Additive exPlanations) values by
solving a weighted linear regression problem that recovers Shapley
values from cooperative game theory. The method uses a weighting kernel
and squared loss to ensure the regression solution satisfies desirable
theoretical properties. It approximates the contribution of each feature
by computing conditional expectations across all possible feature
subsets. Learn more in the [original
paper]((https://arxiv.org/pdf/1705.07874)).

**Advantages** - Theoretically grounded: Satisfies three fundamental
properties: 1. Local accuracy (explanations sum to model output) 2.
Missingness (absent features get zero attribution) 3. consistency
(increasing a feature’s contribution never decreases its attribution) -
Requires less model evaluations than Shapley Value Sampling

**Disadvantages** - Computational complexity, especially with a large
number of features - Assumes feature indepdendence, which may not hold
in practice
...
LIME explains individual predictions by learning a sparse linear model
that locally approximates the complex model’s behavior around the
specific instance being explained. The method perturbs the input by
randomly sampling variations and observes how predictions change, then
learns an interpretable explanation that is locally faithful using
proximity-weighted sampling.

**Advantages** - Model agnostic - Limits explanations to most important
features to maintain human comprehensibility

**Disadvantages** - No additivity - Feature independence assumption
...
historical observations most influence the prediction.
...
functions, like the Normal, Student’s T, etc.

For more information on the different loss functions supported in
*neuralforecast*, read [here](../capabilities/02_objectives).
...
2.5 M     Total params
10.089    Total estimated model params size (MB)
34        Modules in train mode
0         Modules in eval mode
`Trainer.fit` stopped: `max_steps=20` reached.
GPU available: True (mps), used: True
