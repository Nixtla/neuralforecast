...
`load` refuses non-local paths unless you opt in with
`trust_remote=True`, because whoever can write that location chooses
what gets loaded on this machine. Here the artifacts were just written
by this notebook, so opting in is safe.

```python
nf2 = NeuralForecast.load(save_path, trust_remote=True)
```
...
![](./distributed_neuralforecast_files/figure-markdown_strict/cell-14-output-1.png)
