...
-   **Forecast Interpretability**: Plot trend, seasonality and exogenous
    for `NBEATS`, `NHITS`, and `TFT` models. Compute feature
    attributions across all models with `IntegratedGradients` or
    `ShapleyValueSampling` methods.
...
-   **Model Collection**: Out of the box implementation of `MLP`,
    `LSTM`, `RNN`, `TCN`, `DilatedRNN`, `NBEATS`, `NHITS`, `Informer`,
    `TFT`, `PatchTST`, `VanillaTransformer`, `StemGNN` and `HINT`. See
    the entire [collection here](../capabilities/overview.html).
...
![](./introduction_files/figure-markdown_strict/cell-5-output-2.png)
