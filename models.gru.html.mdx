...
<a
href="https://github.com/Nixtla/neuralforecast/blob/main/neuralforecast/models/gru.py#L17"
...
size.<br/> `encoder_activation`: str=`tanh`, type of GRU activation from
`tanh` or `relu`.<br/> `encoder_bias`: bool=True, whether or not to use
biases b_ih, b_hh within GRU units.<br/> `encoder_dropout`: float=0.,
dropout regularization applied to GRU outputs.<br/> `context_size`:
