...
`model` | <code>NeuralForecast model</code> | Instantiated model class from [architecture collection](./models.html). | *required*
`S` | <code>[ndarray](#numpy.ndarray)</code> | Dumming matrix of size (`base`, `bottom`) see HierarchicalForecast's [aggregate method](../hierarchicalforecast/utils.html#aggregate). | *required*
...
Name | Type | Description | Default
---- | ---- | ----------- | -------
`dataset` | <code>[TimeSeriesDataset](#TimeSeriesDataset)</code> | NeuralForecast's `TimeSeriesDataset` see details [here](./tsdataset.html) | *required*
