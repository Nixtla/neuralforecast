...
`decoder_activation` | <code>[str](#str)</code> | activation function for the MLP decoder, see [activations collection](https://docs.pytorch.org/docs/stable/nn.html#non-linear-activations-weighted-sum-nonlinearity). Unused when `decoder_hidden_layers=0`, since the decoder is then a single linear layer. | <code>'ReLU'</code>
