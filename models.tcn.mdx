...
# <kbd>module</kbd> `neuralforecast.models.tcn`
...
**Args:**
 ...
 - <b>`kernel_size`</b> (int):  size of the convolving kernel. 
 - <b>`dilations`</b> (int list):  controls the temporal spacing between the kernel points; also known as the à trous algorithm. 
 - <b>`encoder_hidden_size`</b> (int):  units for the TCN's hidden state size. 
 - <b>`encoder_activation`</b> (str):  type of TCN activation from `tanh` or `relu`. 
