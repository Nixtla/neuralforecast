from neuralforecast.auto import AutoInformer, Informer, RayOptions
...
def test_autoinformer(setup_dataset):
    # Unit test to test that Auto* model contains all required arguments from BaseAuto
    check_args(AutoInformer, exclude_args=['cls_model'])
    ...
    model = AutoInformer(h=12, config=my_config_new, backend='optuna', num_samples=1)
    ...
    model = AutoInformer(h=12, config=my_config, backend='ray', num_samples=1, ray_options=RayOptions(cpus=1))
    model.fit(dataset=setup_dataset)