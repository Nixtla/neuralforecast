from neuralforecast.auto import AutoFEDformer, FEDformer, RayOptions
...
def test_fedformer(suppress_warnings):
    check_model(FEDformer, ["airpassengers"])


def test_autofedformer(setup_dataset):
    # Unit test to test that Auto* model contains all required arguments from BaseAuto
    check_args(AutoFEDformer, exclude_args=['cls_model'])
    ...
    model = AutoFEDformer(h=12, config=my_config_new, backend='optuna', num_samples=1)
    ...
    model = AutoFEDformer(h=12, config=my_config, backend='ray', num_samples=1, ray_options=RayOptions(cpus=1))
    model.fit(dataset=setup_dataset)
