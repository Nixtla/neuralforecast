from neuralforecast.auto import AutoTiDE, RayOptions, TiDE
...
def test_tide_model(suppress_warnings):
    check_model(TiDE, ["airpassengers"])


def test_autotide(setup_dataset):
    # Unit test to test that Auto* model contains all required arguments from BaseAuto
    check_args(AutoTiDE, exclude_args=['cls_model'])
    ...
    model = AutoTiDE(h=12, config=my_config_new, backend='optuna', num_samples=1)
    ...
    model = AutoTiDE(h=12, config=my_config, backend='ray', num_samples=1, ray_options=RayOptions(cpus=1))
    model.fit(dataset=setup_dataset)
