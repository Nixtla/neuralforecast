from neuralforecast.auto import AutoAutoformer, Autoformer, RayOptions
...
def test_autoautoformer(setup_dataset):
    ...
    model = AutoAutoformer(h=12, config=my_config_new, backend='optuna', num_samples=1)
    ...
    model = AutoAutoformer(h=12, config=my_config, backend='ray', num_samples=1, ray_options=RayOptions(cpus=1))
    model.fit(dataset=setup_dataset)