from neuralforecast.auto import AutoTimesNet, RayOptions, TimesNet
...
def test_autotimesnet(setup_dataset):
    ...
    model = AutoTimesNet(h=12, config=my_config_new, backend='optuna', num_samples=1)
    ...
    model = AutoTimesNet(h=12, config=my_config, backend='ray', num_samples=1, ray_options=RayOptions(cpus=1))
    model.fit(dataset=setup_dataset)