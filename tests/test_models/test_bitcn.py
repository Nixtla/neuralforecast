from neuralforecast.auto import AutoBiTCN, BiTCN, RayOptions
...
def test_autobitcn(setup_dataset):
    ...
    model = AutoBiTCN(h=12, config=my_config_new, backend='optuna', num_samples=1)
    ...
    model = AutoBiTCN(h=12, config=my_config, backend='ray', num_samples=1, ray_options=RayOptions(cpus=1))
    model.fit(dataset=dataset)