from neuralforecast.auto import AutoMLPMultivariate, MLPMultivariate, RayOptions
...
def test_mlpmultivariate_model(suppress_warnings):
    check_model(MLPMultivariate, ["airpassengers"])

def test_automlpmultivariate(setup_dataset):
    # Unit test to test that Auto* model contains all required arguments from BaseAuto
    check_args(AutoMLPMultivariate, exclude_args=['cls_model'])
    ...
    model = AutoMLPMultivariate(h=12, n_series=1, config=my_config_new, backend='optuna', num_samples=1)
    ...
    model = AutoMLPMultivariate(h=12, n_series=1, config=my_config, backend='ray', num_samples=1, ray_options=RayOptions(cpus=1))
    model.fit(dataset=setup_dataset)