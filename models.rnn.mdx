...
# <kbd>module</kbd> `neuralforecast.models.rnn`
...
**Args:**
 ...
 - <b>`encoder_n_layers`</b> (int):  number of layers for the RNN. 
 - <b>`encoder_hidden_size`</b> (int):  units for the RNN's hidden state size. 
 - <b>`encoder_activation`</b> (str):  type of RNN activation from `tanh` or `relu`. 
 - <b>`encoder_bias`</b> (bool):  whether or not to use biases b_ih, b_hh within RNN units. 
 - <b>`encoder_dropout`</b> (float):  dropout regularization applied to RNN outputs. context_size (deprecated). 
