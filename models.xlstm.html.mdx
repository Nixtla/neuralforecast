...
`decoder_activation` | <code>[Optional](#typing.Optional)\[[str](#str)\]</code> | activation function for the MLP decoder, one of `ACTIVATIONS` in `neuralforecast.common._modules`. Default None uses 'GELU'. Ignored when `decoder_layers=1`, since the decoder is then a single linear layer. | <code>None</code>
