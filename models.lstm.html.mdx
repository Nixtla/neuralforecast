...
size.<br/> `encoder_bias`: bool=True, whether or not to use biases b_ih,
b_hh within LSTM units.<br/> `encoder_dropout`: float=0., dropout
regularization applied to LSTM outputs.<br/> `context_size`: int=10, size
of context vector for each timestamp on the forecasting window.<br/>
