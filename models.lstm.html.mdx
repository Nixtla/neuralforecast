...
LSTM.<br/> `encoder_hidden_size`: int=200, units for the LSTM’s hidden
state size.<br/> `encoder_bias`: bool=True, whether or not to use biases
b_ih, b_hh within LSTM units.<br/> `encoder_dropout`: float=0., dropout
regularization applied to LSTM outputs.<br/> `context_size`:
