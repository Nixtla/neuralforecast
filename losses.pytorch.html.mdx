...
`return_samples` | <code>[bool](#bool)</code> | Whether or not return raw sample trajectories. When True, the output DataFrame includes columns `"-sample-0"` through `"-sample-{num_samples-1}"`, each representing one full autoregressive trajectory across the forecast horizon. | <code>False</code>
