...
__all__ = ['logger', 'nonzero_indexes_by_row', 'distance_to_holiday', 'make_holidays_distance_df', 'Favorita']
...
from .utils import (
    ...
    check_nans, one_hot_encoding,
    numpy_balance, numpy_ffill, numpy_bfill
)
...
class Favorita:
    ...
    def get_hierarchical_crps(data, Y, Y_hat, q_to_pred, model_name='current'):
        ...
        dpmn_gbu  = [0.1975,0.1365,0.1892,0.1994,0.2649]
        dpmn_nbu  = [0.2653,0.2080,0.2568,0.2663,0.3299]
        hiere2e   = [0.2698,0.2396,0.2664,0.2738,0.3086]
        permbu_mint = [0.2335,0.1346,0.1912,0.3419,0.2766]
        arima_erm = [0.3675,0.2979,0.3593,0.3729,0.4408]
        arima_mint_shr = [0.2606,0.1468,0.2176,0.3769,0.3096]
        ...
        for i, idxs in enumerate(hier_idxs):
            ...
            #n_items,n_time,n_series,n_quant -> n_items*n_series,n_time,n_quant
            y = y.reshape(-1, n_time)
            y_hat = y_hat.reshape(-1, n_time, n_quant)

            crps  = Favorita.get_normalized_crps(y, y_hat, q_to_pred)
    ...
    def get_hierarchical_rmse(data, Y, Y_hat, model_name='current'):
        ...
        hier_idxs = data['hier_idxs']
        hier_levels = data['hier_levels']
        n_items, n_series, n_time = Y_hat.shape

        mqcnn_nbu= [np.nan] * 5
        dpmn_gbu = [np.nan] * 5
        dpmn_nbu = [np.nan] * 5
        hiere2e  = [np.nan] * 5
        arima_mint_shr = [np.nan] * 5
        ...
        for i, idxs in enumerate(hier_idxs):
            ...
            #n_items,n_time,n_series,n_quant -> n_items*n_series,n_time
            y = y.reshape(-1, n_time)
            y_hat = y_hat.reshape(-1, n_time)

            measure = Favorita.get_normalized_rmse(y, y_hat)
