...
class _Concentrator(nn.Module):
    def __init__(self, n_series: int, type: str, treatment_var_name: str):
        super().__init__()
        ...
        self.k_s = nn.Embedding(self.n_series, 1)
        # Initialize with 0.5
        init_k = torch.ones((self.n_series, 1)) * 0.5
        self.k_s.weight.data.copy_(init_k)
    ...
    def forward(
        ...
        idx = idx.long()
        k_s = self.k_s(idx)  # [B,1]
        k_s = torch.squeeze(k_s, -1)
        treatment_var = hist_exog[:, :, -1]  # [B,L,C] -> [B,L]

        # print('k_s', k_s.shape)
        # print('treatment_var', treatment_var.shape)

        # ADD CODE HERE
        treatment = treatment_var * k_s

        # Replace treatment variable with concentration
        hist_exog_out = torch.zeros(hist_exog.shape)
        hist_exog[:, :, :-1] += hist_exog[:, :, :-1]
        hist_exog[:, :, -1] += treatment
...
class NHITSBlock(nn.Module):
    ...
    def forward(
        ...
        # TODO: Add anti-treatment? (remove treatment vars altogether?)
...
# %% ../../nbs/models.nhits_treat.ipynb 11
class NHITS_TREAT(BaseWindows):
    ...
    **References:**<br>
    ...
    def __init__(
        ...
        hist_exog_list=None,
        stat_exog_list=None,
        stack_types: list = ["identity", "identity", "concentrator"],
        n_blocks: list = [1, 1, 1],
