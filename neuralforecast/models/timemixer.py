...
from ..utils import calculate_output_length_1d
    ...
    **References**<br>
    ...
    def __init__(
        ...
        **trainer_kwargs,
        ...
        self.down_sampling_module = {}

        low_features, high_features = self._layered_features(
            self.input_size,
            self.down_sampling_method,
    ...
    def _layered_features(
        self, seq_len, down_sampling_method, down_sampling_window, down_sampling_layers
    ):
        low_features = [seq_len]
        high_features = []
        out_len = seq_len
        for i in range(down_sampling_layers):
            if down_sampling_method == "conv":
                out_len = calculate_output_length_1d(
                    input_length=out_len,
                    padding=1,
                    kernel_size=3,
                    stride=down_sampling_window,
                )
            else:
                kernel_size = min(down_sampling_window, out_len)
                out_len = calculate_output_length_1d(
                    input_length=out_len,
                    padding=0,
            ...
            if i < down_sampling_layers - 1:
                low_features.append(out_len)
            high_features.append(out_len)
        return low_features, high_features

    def _down_sampling_module(self, kernel_size):
        kernel_size = min(self.down_sampling_window, kernel_size)
        if self.down_sampling_method == "max":
            if kernel_size not in self.down_sampling_module:
                down_pool = torch.nn.MaxPool1d(kernel_size, return_indices=False)
                self.down_sampling_module[kernel_size] = down_pool
        elif self.down_sampling_method == "avg":
            if kernel_size not in self.down_sampling_module:
                down_pool = torch.nn.AvgPool1d(kernel_size)
                self.down_sampling_module[kernel_size] = down_pool
            ...
            if kernel_size not in self.down_sampling_module:
                ...
                self.down_sampling_module[kernel_size] = down_pool
        return self.down_sampling_module[kernel_size]
    ...
    def __multi_scale_process_inputs(self, x_enc, x_mark_enc):
        if self.down_sampling_method not in ("max", "avg", "conv"):
        ...
        for i in range(self.down_sampling_layers):
            x_enc_sampling = self._down_sampling_module(x_enc_ori.shape[-1])(x_enc_ori)
