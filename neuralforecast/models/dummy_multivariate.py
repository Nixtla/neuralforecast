...
class DummyMultivariate(BaseModel):
    ...
    It simply feeds back the last input values as predictions for each series.
    It's designed to test the multivariate recurrent prediction infrastructure.
    
    **Parameters:**<br>
    `h`: int, forecast horizon.<br>
    `input_size`: int, maximum sequence length for truncated train backpropagation.<br>
    `n_series`: int, number of time series in the multivariate dataset.<br>
    ...
    def __init__(
        ...
        **trainer_kwargs
        ...
        self.dummy_layer = nn.Linear(n_series, n_series)
    ...
    def forward(self, windows_batch):
        ...
        and uses it as the prediction for each series.
        """
        ...
        for i in range(self.h):
            ...
            # Get the seasonal value for all series
            seasonal_value = insample_y[:, seasonal_idx, :]  # [B, N]
            # Apply dummy layer to simulate some "learning" across series
            seasonal_value = self.dummy_layer(seasonal_value)
