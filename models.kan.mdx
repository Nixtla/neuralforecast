...
# <kbd>module</kbd> `neuralforecast.models.kan`
...
#### <kbd>property</kbd> scaled_spline_weight
...
**Args:**
 ...
 - <b>`x`</b> (torch.Tensor):  Input tensor of shape (batch_size, in_features). 



**Returns:**
 
 - <b>`torch.Tensor`</b>:  B-spline bases tensor of shape (batch_size, in_features, grid_size + spline_order). 

---



### <kbd>method</kbd> `curve2coeff`
...
**Args:**
 ...
 - <b>`x`</b> (torch.Tensor):  Input tensor of shape (batch_size, in_features). 
 - <b>`y`</b> (torch.Tensor):  Output tensor of shape (batch_size, in_features, out_features). 



**Returns:**
 
 - <b>`torch.Tensor`</b>:  Coefficients tensor of shape (out_features, in_features, grid_size + spline_order). 
...
### <kbd>method</kbd> `regularization_loss`
...
### <kbd>method</kbd> `reset_parameters`
...
**Args:**
 ...
 - <b>`input_size`</b> (int):  considered autorregresive inputs (lags), y=[1,2,3,4] input_size=2 -> lags=[1,2].<br/> 
 - <b>`grid_size`</b> (int):  number of intervals used by the splines to approximate the function. 
 - <b>`spline_order`</b> (int):  order of the B-splines. 
 - <b>`scale_noise`</b> (float):  regularization coefficient for the splines. 
 - <b>`scale_base`</b> (float):  scaling coefficient for the base function. 
 - <b>`scale_spline`</b> (float):  scaling coefficient for the splines. 
 - <b>`enable_standalone_scale_spline`</b> (bool):  whether each spline is scaled individually. 
 - <b>`grid_eps`</b> (float):  used for numerical stability. 
 - <b>`grid_range`</b> (list):  range of the grid used for spline approximation. 
 - <b>`n_hidden_layers`</b> (int):  number of hidden layers for the KAN. 
 - <b>`hidden_size`</b> (int or list):  number of units for each hidden layer of the KAN. If an integer, all hidden layers will have the same size. Use a list to specify the size of each hidden layer. 
 ...
 - <b>`loss`</b> (PyTorch module):  instantiated train loss class from [losses collection](https://nixtlaverse.nixtla.io/neuralforecast/losses.pytorch). 
 - <b>`valid_loss`</b> (PyTorch module):  instantiated valid loss class from [losses collection](https://nixtlaverse.nixtla.io/neuralforecast/losses.pytorch). 
 ...
 - <b>`scaler_type`</b> (str):  type of scaler for temporal inputs normalization see [temporal scalers](https://nixtlaverse.nixtla.io/neuralforecast/common.scalers). 
