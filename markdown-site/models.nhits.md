...
(NHITS). `NHITS` builds upon `NBEATS` and specializes its partial
...
long-horizon forecasting task `NHITS` improved accuracy by 25% on AAAI’s
best paper award the `Informer`, while being 50x faster.
...
![Figure 1. Neural Hierarchical Interpolation for Time Series
(NHITS).](imgs_models/nhits.png)
