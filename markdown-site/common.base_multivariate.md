...
> The `BaseWindows` class contains standard methods shared across
> window-based multivariate neural networks; in contrast to recurrent
> neural networks these models commit to a fixed sequence length input.
> <br><br>The standard methods include data preprocessing
> `_normalization`, optimization utilities like parameter
> initialization, `training_step`, `validation_step`, and shared `fit`
> and `predict` methods.These shared methods enable all the
> `neuralforecast.models` compatibility with the `core.NeuralForecast`
> wrapper class.
...
class BaseMultivariate(pl.LightningModule):
    ...
    def _inv_normalization(self, y_hat, temporal_cols):
        ...
        y_scale = self.scaler.x_scale[:,temporal_data_cols.get_indexer(['y']), :].squeeze(1)
        y_loc = self.scaler.x_shift[:,temporal_data_cols.get_indexer(['y']), :].squeeze(1)
