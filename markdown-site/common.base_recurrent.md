...
states. The class is represented by `LSTM`, `GRU`, and `RNN`, along with
other more sophisticated architectures like `MQCNN`.
...
class BaseRecurrent(pl.LightningModule):
    ...
    def _inv_normalization(self, y_hat, temporal_cols):
        ...
        y_loc = self.scaler.x_shift[:, temporal_data_cols.get_indexer(['y']), 0].flatten() #[B,C,T] -> [B]        
