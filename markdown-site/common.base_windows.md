...
> window-based neural networks; in contrast to recurrent neural networks
> these models commit to a fixed sequence length input. The class is
> represented by `MLP`, and other more sophisticated architectures like
> `NBEATS`, and `NHITS`.<br><br>The standard methods include data
> preprocessing `_normalization`, optimization utilities like parameter
