...
# <kbd>module</kbd> `neuralforecast.models.tsmixer`
...
```python
__init__(n_series, input_size, dropout)
```
...
```python
__init__(n_series, input_size, dropout, ff_dim)
```
...
**Args:**
 ...
 - <b>`exclude_insample_y`</b> (bool):  if True excludes the target variable from the input features. 
 ...
 - <b>`revin`</b> (bool):  if True uses Reverse Instance Normalization to process inputs and outputs. 
