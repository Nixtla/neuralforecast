...
from neuralforecast.losses.pytorch import MAE, MQLoss, IQLoss, MSE, HuberLoss, HuberMQLoss, GMM, IQLoss, HuberIQLoss, DistributionLoss, GMM
...
def cross_validation(dataset, horizon, metrics, seed=1234567, path_prefix="s3://timenet/isf2025/gmm"):
    ...
    for model in models:
        ...
        level = [10, 20, 30, 40, 50, 60, 70, 80, 90, 95, 98, 99]
        ...
        # cast all float columns to f32
        eval_df = evaluate(df=Y_hat_df.drop("cutoff").to_pandas(),
                        train_df=train_df[[id_col, time_col, target_col]].to_pandas(),
                        metrics=metrics_,
                        level=level,
                        agg_fn="mean",
                        )
        eval_df = pl.from_pandas(eval_df)
