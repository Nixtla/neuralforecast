...
`use_init_models` | <code>[bool](#bool)</code> | If True, discards any previously fitted weights and reinitializes the models from the configs passed at `NeuralForecast(__init__)`. Use this to start training from scratch. Defaults to False. | <code>False</code>
...
```python
load(path, verbose=False, allow_pickle=False, trust_remote=False, **kwargs)
```
...
`allow_pickle` | <code>[bool](#bool)</code> | Permit the unrestricted legacy read. Defaults to False. | <code>False</code>
`trust_remote` | <code>[bool](#bool)</code> | Permit loading from a non-local path. Defaults to False. | <code>False</code>
`**kwargs` | | Additional keyword arguments to be passed to each model's `load`. | <code>{}</code>
