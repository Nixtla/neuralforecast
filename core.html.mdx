...
```python
NeuralForecast(models, freq, local_scaler_type=None)
```
...
`local_scaler_type` | <code>[str](#str)</code> | Scaler to apply per-serie to all features before fitting, which is inverted after predicting. Can be 'standard', 'robust', 'robust-iqr', 'minmax' or 'boxcox'. Defaults to None. | <code>None</code>
